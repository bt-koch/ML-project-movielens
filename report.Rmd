...
five starts can be awarded. A one-star rating indicates the worst possible rating,
while a five-star rating corresponds to the best possible rating.
...
combination and it is summed over all these combinations. The RMSE is the typical
error that is made when predicting a movie rating and can therefore be interpreted
similarly to a standard deviation. If the RMSE exceeds a value of one, a typical
...
how much ratings where made per movie. Some movies have been rated by very few
...
$$\frac{1}{N}\sum_{u,i}(y_{u,i} - \mu - b_i - b_u)^2 + \lambda \left( \sum_i b^2_i + \sum_u b^2_u + \sum_d b^2_d \right)$$
...
and certain groups of users have similar rating patterns. With the use of matrix
factorization it is attempted to capture these patterns and describe them by 
factors. The idea behind matrix factorization is to approximate the whole rating
matrix $\boldsymbol{R}_{m \times n}$ by the product of two matrices of lower
dimensions ($\boldsymbol{P}_{n \times k}$ and $\boldsymbol{Q}_{n \times k}$)
so that
...
<!-- # create matrix where columns = movieId and rows = userId with filtered train data -->
<!-- matrix_mf <- train_set %>% -->
<!--   group_by(movieId) %>% -->
<!--   filter(n() >= 100) %>% -->
<!--   ungroup() %>% -->
...
<!-- # userId as row name -->
<!-- rownames(matrix_mf) <- matrix_mf[,1] -->
<!-- matrix_mf <- matrix_mf[,-1] -->

<!-- # title as column name -->
<!-- titles <- train_set %>% select(movieId, title) %>% distinct() -->
<!-- colnames(matrix_mf) <- with(titles, title[match(colnames(matrix_mf), movieId)]) -->

<!-- # calculate residuals -->
<!-- matrix_mf <- sweep(matrix_mf, 2, colMeans(matrix_mf, na.rm = TRUE)) -->
<!-- matrix_mf <- sweep(matrix_mf, 1, rowMeans(matrix_mf, na.rm = TRUE)) -->

<!-- m1 <- "Star Wars: Episode IV - A New Hope (a.k.a. Star Wars) (1977)" -->
<!-- m11 <- "Star Wars: Episode V - The Empire Strikes Back (1980)" -->
<!-- m2 <- "Star Trek: The Motion Picture (1979)" -->

<!-- m3 <- "Resident Evil (2002)" -->
<!-- m4 <- "Shaun of the Dead (2004)" -->

<!-- m5 <- "Catch and Release (2006)" -->
<!-- m6 <- "Definitely, Maybe (2008)" -->

<!-- m7 <- "Godfather, The (1972)" -->
<!-- m71 <- "Godfather: Part II, The (1974)" -->
<!-- m8 <- "Sleepless in Seattle (1993)" -->

<!-- a <- "Godfather, The (1972)" -->
<!-- b <- "Godfather: Part II, The (1974)" -->
<!-- c <- "Goodfellas (1990)" -->
<!-- d <- "You've Got Mail (1998)" -->
<!-- e <- "Sleepless in Seattle (1993)" -->


<!-- # test <- cor(matrix_mf, use="pairwise.complete") -->


<!-- p1 <- qplot(matrix_mf[,m1], matrix_mf[,m11]) -->
<!-- p2 <- qplot(matrix_mf[,m1], matrix_mf[,m2]) -->
<!-- p3 <- qplot(matrix_mf[,d], matrix_mf[,e]) -->

<!-- gridExtra::grid.arrange(p1,p2,p3,ncol=3) -->

<!-- x <- matrix_mf[, c(m1,m11,m2,m3,m4,m5,m6,m7,m71,m8)] -->
<!-- colnames(x) <- c("star wars", "sw2", "star trek", "resident", "shaun", "catch", "definitely", "godfather", "godf2", "sleepless") -->
<!-- cor(x, use="pairwise.complete") -->

<!-- xdf <- as.data.frame(test) -->
<!-- xdf <- xdf[xdf < -0.8] -->
<!-- xdf <- xdf[is.na(xdf) == F] -->
<!-- ```  -->
...
regularized linear models according to the `edx` dataset, the matrix
factorization model with the tuned parameters is chosen to be applied to the
`validation` dataset.

<!-- ```{r} -->

<!-- result <- expand.grid(user = unique(validation$userId), movie = unique(validation$movieId)) -->


<!-- ggplot(predicted_ratings, aes(x = )) -->
<!-- ``` -->
